    ...
    public class MqttPacketSerializer : IMqttPacketSerializer
    {
        ...
        private byte SerializePacket(MqttBasePacket packet, MqttPacketWriter packetWriter)
        {
            if (packet is MqttConnectPacket)
            {
                ...
                return Serialize(connectPacket, packetWriter);
            }
            else if (packet is MqttConnAckPacket)
            {
                var connAckPacket = packet as MqttConnAckPacket;
                return Serialize(connAckPacket, packetWriter);
            }
            else if (packet is MqttDisconnectPacket)
            {
                var unsubscribePacket = packet as MqttDisconnectPacket;
                return SerializeEmptyPacket(MqttControlPacketType.Disconnect);
            }
            else if (packet is MqttPingReqPacket)
            {
                var unsubscribePacket = packet as MqttPingReqPacket;
            ...
            else if (packet is MqttPublishPacket)
            {
                var publishPacket = packet as MqttPublishPacket;
                return Serialize(publishPacket, packetWriter);
            }
            else if (packet is MqttPubAckPacket)
            {
                var pubAckPacket = packet as MqttPubAckPacket;
                return Serialize(pubAckPacket, packetWriter);
            }
            else if (packet is MqttPubRecPacket)
            {
                var pubRecPacket = packet as MqttPubRecPacket;
                return Serialize(pubRecPacket, packetWriter);
            }
            else if (packet is MqttPubRelPacket)
            {
                var pubRelPacket = packet as MqttPubRelPacket;
                return Serialize(pubRelPacket, packetWriter);
            }
            else if (packet is MqttPubCompPacket)
            {
                var pubCompPacket = packet as MqttPubCompPacket;
                return Serialize(pubCompPacket, packetWriter);
            }
            else if (packet is MqttSubscribePacket)
            {
                var subscribePacket = packet as MqttSubscribePacket;
                return Serialize(subscribePacket, packetWriter);
            }
            else if (packet is MqttSubAckPacket)
            {
                var subAckPacket = packet as MqttSubAckPacket;
                return Serialize(subAckPacket, packetWriter);
            }
            else if (packet is MqttUnsubscribePacket)
            {
                var unsubscribePacket = packet as MqttUnsubscribePacket;
                return Serialize(unsubscribePacket, packetWriter);
            }
            else if (packet is MqttUnsubAckPacket)
            {
                var unsubAckPacket = packet as MqttUnsubAckPacket;
                return Serialize(unsubAckPacket, packetWriter);
            }
            else
